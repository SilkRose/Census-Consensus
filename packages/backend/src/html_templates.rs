			...
			body {
				form method = "post" action = "/user-feedback" {
					...
					@if let Some(public_feedback) = public_feedback {
							textarea id = "public" type = "text" name = "feedback_public" cols = "30" rows = "10" value = (public_feedback) {}
					} @else {
					...
					@if let Some(private_feedback) = private_feedback {
							textarea id = "private" type = "text" name = "feedback_private" cols = "30" rows = "10" value = (private_feedback) {}
					} @else {
